    ...
    <article>
      ...
      <div className="border border-border bg-bg/30 px-4 py-3 mb-8 text-[13px]">
        {pin.occupier_id ? (
          ...
          <div>분양가 <b className="text-navy">{Number(pin.occupy_price).toLocaleString()} mlbg</b> · 일 수익 {pin.daily_income} mlbg · <a href={`/?restaurant=${pin.id}`} target="_top" className="text-cyan underline whitespace-nowrap">지도에서 분양받기</a></div>
        )}
      </div>
