...
export default function StadiumEditForm({ pin, currentUserId }: { pin: Pin; currentUserId: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (busy) return;
    setErr(null);
    if (!name.trim() || !description.trim() || !recommendedActivity.trim()) { setErr('경기장명/설명/대표 종목 모두 필수'); return; }
    setBusy(true);
    ...
    const { data, error } = await supabase.rpc('update_stadium_pin', {
