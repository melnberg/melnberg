...
export default function FactoryPanel({ factory, onClose, onChanged }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) setComments((data ?? []) as FactoryComment[]);
    })();
            ...
            {factory.occupier_id ? (
              <>
                ...
                <div className="flex items-baseline justify-between gap-3 mb-2">
                  ...
                  <span className="text-[20px] font-black text-cyan tabular-nums">{totalEarned.toLocaleString()} <span className="text-[12px] text-muted">mlbg</span></span>
                </div>
                {isMine && (
                  <div className="flex items-baseline justify-between gap-3 pt-2 border-t border-black/10">
                    ...
                    <span className="text-[18px] font-black text-[#dc2626] tabular-nums">{owedAmount.toLocaleString()} <span className="text-[12px]">mlbg</span></span>
                  </div>
                )}
                {factory.occupied_at && <div className="text-[10px] text-muted mt-2">분양일: {fmtKstShort(factory.occupied_at)}</div>}
              </>
            ) : (
              <>
