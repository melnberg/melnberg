  ...
  return (
    <>
      <aside
        ...
        className={`fixed lg:sticky top-0 left-0 z-50 w-[240px] lg:w-[220px] h-[100dvh] lg:h-screen flex-shrink-0 bg-white border-r border-border flex flex-col overflow-y-auto transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${open ? 'shadow-[4px_0_16px_rgba(0,0,0,0.08)]' : ''}`}
      >
        ...
        {/* 사이드바 맨 아래 — 사업자 정보 (심사 대비 검정, 컴팩트) */}
        <div className="mt-auto border-t border-border px-3 py-2 text-[11px] leading-tight text-black font-medium">
          <p>안세 · 대표 임은종</p>
...
// 일깃장 — 책 아이콘 (열린 페이지 + 줄). 이전 ThreadsIcon (@ 기호) 은 라벨 변경 ("나의 일깃장") 으로 폐기.
// 굴뚝 있는 집 — 스레드 메뉴 (각자의 일기장 = 자기 집 비유). HomeIcon 과 구별 위해 굴뚝 추가.
const DiaryIcon = () => <svg {...iconProps}><path d="M3 11l9-8 9 8v10a2 2 0 0 1-2 2h-4v-7h-6v7H5a2 2 0 0 1-2-2z" /><path d="M17 6V3h2v5" /></svg>;
...
// 동전(원형 + B) — 코인 토론
const CoinIcon = () => <svg {...iconProps}><circle cx="12" cy="12" r="9" /><path d="M9 8h4a2 2 0 0 1 0 4H9z" /><path d="M9 12h4.5a2 2 0 0 1 0 4H9z" /><line x1="11" y1="6" x2="11" y2="8" /><line x1="11" y1="16" x2="11" y2="18" /></svg>;
