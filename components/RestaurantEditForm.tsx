...
export default function RestaurantEditForm({ pin, currentUserId }: { pin: Pin; currentUserId: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (busy) return;
    setErr(null);
    if (!name.trim() || !description.trim() || !recommendedMenu.trim()) {
      setErr('가게명/설명/추천메뉴 모두 필수');
      return;
    }
    ...
    const { data, error } = await supabase.rpc('update_restaurant_pin', {
      p_pin_id: pin.id,
      p_name: name.trim(),
    ...
    alert('수정 완료');
    revalidateHome();
    router.push(`/restaurants/${pin.id}`);
    router.refresh();
  }
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-1">
        ...
        <textarea value={recommendedMenu} onChange={(e) => setRecommendedMenu(e.target.value)} maxLength={200} rows={3}
          className="border border-border px-3 py-2 text-[14px] outline-none focus:border-navy resize-y leading-relaxed" />
      ...
      <div className="flex justify-between gap-3 mt-2">
        <button type="button" onClick={handleDelete} disabled={busy}
          className="bg-white border border-red-300 text-red-600 px-4 py-2 text-[12px] font-bold cursor-pointer hover:bg-red-50 disabled:opacity-50">
          🗑 핀 삭제
        </button>
        <div className="flex gap-3">
          <button type="button" onClick={() => router.back()}
            className="bg-white border border-border text-text px-5 py-2 text-[13px] font-semibold cursor-pointer hover:border-navy hover:text-navy">
            취소
          </button>
          <button type="submit" disabled={busy}
            className="bg-navy text-white border-none px-6 py-2 text-[13px] font-bold tracking-wider uppercase cursor-pointer hover:bg-navy-dark disabled:opacity-50">
            {busy ? '저장 중...' : '저장'}
          </button>
        </div>
      </div>
