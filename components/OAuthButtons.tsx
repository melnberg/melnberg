...
type Provider = 'kakao' | 'google' | 'naver' | 'email';
const LAST_KEY = 'mlbg.lastLoginMethod';

export function recordLastLoginMethod(method: Provider) {
  try { localStorage.setItem(LAST_KEY, method); } catch {}
}
...
export default function OAuthButtons({ next = '/' }: { next?: string }) {
  ...
  const [last, setLast] = useState<Provider | null>(null);

  useEffect(() => {
    try { setLast(localStorage.getItem(LAST_KEY) as Provider | null); } catch {}
  }, []);
  ...
  async function startSupabase(provider: 'kakao' | 'google') {
    ...
    setBusy(provider);
    setErr(null);
    recordLastLoginMethod(provider);
  ...
  return (
    <div className="flex flex-col gap-2">
      <ButtonWithBadge show={last === 'kakao'}>
        <button
          type="button"
      ...
      {false && (
        <ButtonWithBadge show={last === 'naver'}>
          <button
            type="button"
            onClick={startNaver}
            disabled={!!busy}
      ...
      <ButtonWithBadge show={last === 'google'}>
        <button
          type="button"
...
// "최근 사용" 말풍선 — 우측 상단에서 살짝 튀어나오게
function ButtonWithBadge({ show, children }: { show: boolean; children: React.ReactNode }) {
  return (
    <div className="relative">
      {children}
      {show && (
        <div className="absolute -top-2 -right-1 bg-navy text-white text-[10px] font-bold px-2 py-1 leading-none shadow-md whitespace-nowrap pointer-events-none">
          최근 사용
          <div className="absolute -bottom-[3px] right-3 w-2 h-2 bg-navy rotate-45" />
        </div>
      )}
    </div>
  );
