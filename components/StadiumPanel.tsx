...
import { StadiumIcon } from './CategoryIcons';
import FacilityIncomeHistory from './FacilityIncomeHistory';

export type StadiumItem = {
...
export default function StadiumPanel({
  stadium, onClose, onChanged,
}: { stadium: StadiumItem; onClose: () => void; onChanged: () => void }) {
  const supabase = createClient();
  const [me, setMe] = useState<{ id: string; name: string; balance: number } | null>(null);
  const [liked, setLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(stadium.like_count);
  const [comments, setComments] = useState<Comment[] | null>(null);
  const [commentText, setCommentText] = useState('');
  const [busy, setBusy] = useState(false);

  useEffect(() => { setLikeCount(stadium.like_count); }, [stadium.id, stadium.like_count]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (user) {
        ...
        const { data: lk } = await supabase.from('stadium_pin_likes')
          .select('user_id').eq('pin_id', stadium.id).eq('user_id', user.id).maybeSingle();
        if (!cancelled) setLiked(!!lk);
      }
      const { data: cms } = await supabase
        .from('stadium_pin_comments')
        .select('id, author_id, content, created_at')
        .eq('pin_id', stadium.id).is('deleted_at', null)
  ...
  const isMine = !!me && me.id === stadium.occupier_id;
  const isAuthor = !!me && me.id === stadium.author_id;
  ...
  async function toggleLike() {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('toggle_stadium_pin_like', { p_pin_id: stadium.id });
    setBusy(false);
  ...
  async function occupy() {
    if (!me) { alert('로그인 필요'); return; }
    if (!confirm(`${stadium.occupy_price} mlbg 로 분양받습니다.`)) return;
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('occupy_stadium_pin', { p_pin_id: stadium.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
    ...
    alert(`${stadium.name} 분양 완료. -${row.out_paid} mlbg`);
    onChanged();
  }
  ...
  async function handleDelete() {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('delete_stadium_pin', { p_pin_id: stadium.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function release() {
    if (!confirm(`보유 해제 시 ${stadium.occupy_price} mlbg 환불됨.`)) return;
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('release_stadium_pin', { p_pin_id: stadium.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function buy() {
    if (!me) { alert('로그인 필요'); return; }
    if (stadium.listing_price == null) return;
    if (!confirm(`${stadium.listing_price} mlbg 로 매수합니다.`)) return;
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('buy_stadium_pin', { p_pin_id: stadium.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function claim() {
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('claim_stadium_pin_income', { p_pin_id: stadium.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function submitComment(e: React.FormEvent) {
    ...
    const { data, error } = await supabase
      .from('stadium_pin_comments')
      .insert({ pin_id: stadium.id, author_id: me.id, content: c })
    ...
    setCommentText('');
    await awardMlbg('stadium_comment', row.id, c);
  }

  return (
    <div data-side-panel className="absolute top-4 right-4 z-[60] bg-white border-2 border-navy shadow-2xl w-[400px] max-w-[calc(100vw-32px)] max-h-[calc(100vh-32px)] overflow-y-auto">
      <div className="bg-[#bfdbfe] text-[#1a1d22] px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <StadiumIcon className="w-[20px] h-[20px]" />
          <div className="min-w-0">
            <div className="font-bold text-[15px] truncate">{stadium.dong ? `${stadium.dong} ${stadium.name}` : stadium.name}</div>
            {stadium.address && <div className="text-[10px] text-[#1e3a8a] truncate">{stadium.address}</div>}
          </div>
        </div>
      ...
      <div className="p-4">
        {stadium.photo_url && (
          <div className="aspect-square w-full bg-[#f0f0f0] rounded-xl overflow-hidden border border-border mb-3">
            <img src={stadium.photo_url} alt="" className="w-full h-full object-cover" />
          </div>
        )}
        <div className="text-[12px] text-text leading-relaxed mb-2 whitespace-pre-wrap">{stadium.description}</div>
        <div className="border-l-4 border-[#3b82f6] bg-[#dbeafe] px-3 py-2 mb-3">
          <div className="text-[10px] font-bold tracking-widest uppercase text-[#3b82f6] mb-0.5">대표 종목</div>
          <div className="text-[12px] text-text leading-snug whitespace-pre-wrap">{stadium.recommended_activity}</div>
        </div>

        <div className="flex items-center justify-between text-[11px] text-muted mb-3">
          <span className="flex items-center gap-2">
            <span>등록 by <b className="text-navy">{stadium.author_name ?? '익명'}</b></span>
            <span className="tabular-nums">+30 mlbg</span>
            {isAuthor && (<a href={`/stadiums/${stadium.id}/edit`} className="text-cyan underline hover:text-navy no-underline">✏ 수정</a>)}
        ...
        <div className="border border-border bg-bg/30 px-3 py-2 mb-3 text-[12px]">
          {stadium.occupier_id ? (
            <div className="flex items-center justify-between gap-2">
              <span className="text-muted">분양: <b className="text-navy">{stadium.occupier_name ?? '익명'}</b></span>
              {isMine ? (
                <div className="flex flex-col gap-2 w-full">
                  ...
                  <FacilityIncomeHistory type="stadium" id={stadium.id} />
                </div>
              ) : stadium.listing_price != null ? (
                <button onClick={buy} disabled={busy} className="bg-[#dc2626] text-white px-3 py-1 text-[11px] font-bold cursor-pointer hover:bg-[#b91c1c] border-none disabled:opacity-40">
                  매수 ({Number(stadium.listing_price).toLocaleString()} mlbg)
                </button>
              ) : null}
            </div>
          ) : (
            <button onClick={occupy} disabled={busy} className="w-full bg-navy text-white px-3 py-2 text-[12px] font-black border-none cursor-pointer hover:bg-navy-dark disabled:opacity-40">
              🚩 분양받기 ({Number(stadium.occupy_price).toLocaleString()} mlbg) — 일 수익 {stadium.daily_income} mlbg
            </button>
          )}
        </div>
