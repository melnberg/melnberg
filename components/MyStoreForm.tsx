  ...
  function search() {
    ...
    ps.keywordSearch(searchQuery.trim(), (data, status) => {
      ...
      results.forEach((p, idx) => {
        ...
        marker.setMap(map);
        const markerAny = marker as unknown as { _placeIdx?: number };
        markerAny._placeIdx = idx;
      ...
      if (results.length > 0) {
        ...
        // max-zoom 제한 — 너무 가까이 줌인 되면 단일 결과 등에서 어색
            ...
            {searchResults.map((p, idx) => (
              <li key={p.id}>
                <button type="button" onClick={() => pickPlace(p)}
                  ...
                  <span className="flex-shrink-0 inline-flex items-center justify-center w-5 h-5 rounded-full bg-cyan text-white text-[10px] font-black tabular-nums" style={{ background: hoveredIdx === idx ? '#002060' : '#00B0F0' }}>{idx + 1}</span>
