...
export default function RestaurantPanel({
  ...
  const [likeCount, setLikeCount] = useState(restaurant.like_count);
  const [comments, setComments] = useState<Comment[] | null>(null);
  const [commentText, setCommentText] = useState('');
  const [busy, setBusy] = useState(false);

  useEffect(() => { setLikeCount(restaurant.like_count); }, [restaurant.id, restaurant.like_count]);
    ...
    (async () => {
      ...
      if (user) {
        ...
        const { data: lk } = await supabase.from('restaurant_pin_likes')
          .select('user_id').eq('pin_id', restaurant.id).eq('user_id', user.id).maybeSingle();
      ...
      const { data: cms } = await supabase
        ...
        .eq('pin_id', restaurant.id).is('deleted_at', null)
  ...
  const isMine = !!me && me.id === restaurant.occupier_id;
  const isAuthor = !!me && me.id === restaurant.author_id;
  ...
  async function toggleLike() {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('toggle_restaurant_pin_like', { p_pin_id: restaurant.id });
    setBusy(false);
  ...
  async function occupy() {
    if (!me) { alert('로그인 필요'); return; }
    if (!confirm(`${restaurant.occupy_price} mlbg 로 분양받습니다.`)) return;
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('occupy_restaurant_pin', { p_pin_id: restaurant.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
    ...
    alert(`${restaurant.name} 분양 완료. -${row.out_paid} mlbg`);
    onChanged();
  }
  ...
  async function release() {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('release_restaurant_pin', { p_pin_id: restaurant.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function buy() {
    if (!me) { alert('로그인 필요'); return; }
    if (restaurant.listing_price == null) return;
    if (!confirm(`${restaurant.listing_price} mlbg 로 매수합니다.`)) return;
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('buy_restaurant_pin', { p_pin_id: restaurant.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function claim() {
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('claim_restaurant_pin_income', { p_pin_id: restaurant.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function submitComment(e: React.FormEvent) {
    ...
    const { data, error } = await supabase
      .from('restaurant_pin_comments')
      .insert({ pin_id: restaurant.id, author_id: me.id, content: c })
    ...
    <div data-side-panel className="absolute top-4 right-4 z-[60] bg-white border-2 border-navy shadow-2xl w-[400px] max-w-[calc(100vw-32px)] max-h-[calc(100vh-32px)] overflow-y-auto">
      <div className="bg-[#fbbf24] text-[#1a1d22] px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <RestaurantIcon className="w-[20px] h-[20px]" />
      ...
      <div className="p-4">
        ...
        <div className="border-l-4 border-cyan bg-cyan/5 px-3 py-2 mb-3">
          <div className="text-[10px] font-bold tracking-widest uppercase text-cyan mb-0.5">추천메뉴</div>
          <div className="text-[12px] text-text leading-snug whitespace-pre-wrap">{restaurant.recommended_menu}</div>
        </div>
        ...
        <div className="border border-border bg-bg/30 px-3 py-2 mb-3 text-[12px]">
          {restaurant.occupier_id ? (
            <div className="flex items-center justify-between gap-2">
              ...
              {isMine ? (
                <div className="flex flex-col gap-2 w-full">
                  ...
                  <FacilityIncomeHistory type="restaurant" id={restaurant.id} />
                </div>
                ...
                <button onClick={buy} disabled={busy} className="bg-[#dc2626] text-white px-3 py-1 text-[11px] font-bold cursor-pointer hover:bg-[#b91c1c] border-none disabled:opacity-40">
                  매수 ({Number(restaurant.listing_price).toLocaleString()} mlbg)
                </button>
              ) : null}
            </div>
          ) : (
            <button onClick={occupy} disabled={busy} className="w-full bg-navy text-white px-3 py-2 text-[12px] font-black border-none cursor-pointer hover:bg-navy-dark disabled:opacity-40">
              🚩 분양받기 ({Number(restaurant.occupy_price).toLocaleString()} mlbg) — 일 수익 {restaurant.daily_income} mlbg
            </button>
          )}
        </div>
        ...
        <div className="mt-3 border-t border-border pt-3">
          ...
          {comments === null ? (
            ...
            <p className="text-[11px] text-muted text-center py-3">첫 댓글을 남겨보세요. (+0.5 mlbg)</p>
          ) : (
