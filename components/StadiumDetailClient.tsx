...
import type { StadiumItem } from './StadiumPanel';
...
export default function StadiumDetailClient({ pin }: { pin: StadiumItem }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: cms } = await supabase.from('stadium_pin_comments')
        .select('id, author_id, content, created_at')
        .eq('pin_id', pin.id).is('deleted_at', null).order('created_at', { ascending: true });
  ...
  async function toggleLike() {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('toggle_stadium_pin_like', { p_pin_id: pin.id });
    setBusy(false);
  ...
  async function handleDelete() {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('delete_stadium_pin', { p_pin_id: pin.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function occupy() {
    ...
    setBusy(true);
    const { data, error } = await supabase.rpc('occupy_stadium_pin', { p_pin_id: pin.id });
    setBusy(false);
    if (error) { alert(error.message); return; }
  ...
  async function submitComment(e: React.FormEvent) {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.from('stadium_pin_comments')
    ...
    <article>
      <header className="pb-4 mb-6 border-b border-border">
        <div className="flex items-baseline gap-2 mb-2 flex-wrap">
          <StadiumIcon className="w-[20px] h-[20px] text-navy" />
        ...
        <div className="text-[11px] text-muted mt-1 flex items-center gap-2">
          ...
          {isAuthor && <Link href={`/stadiums/${pin.id}/edit`} className="text-cyan underline hover:text-navy no-underline">✏ 수정</Link>}
      ...
      <div className="border-l-4 border-[#3b82f6] bg-[#dbeafe] px-4 py-3 mb-6">
        <div className="text-[11px] font-bold tracking-widest uppercase text-[#3b82f6] mb-1">대표 종목</div>
        <div className="text-[14px] text-text leading-relaxed whitespace-pre-wrap">{pin.recommended_activity}</div>
      </div>
        ...
        {occupier ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <a href={`/?stadium=${pin.id}&lat=${pin.lat}&lng=${pin.lng}`} target="_top" className="bg-white text-navy border border-navy px-4 py-2.5 text-[12px] font-bold no-underline cursor-pointer hover:bg-navy-soft inline-flex items-center justify-center gap-1.5 whitespace-nowrap">
      ...
      <div className="border-t border-border pt-6">
        ...
        {comments === null ? (<p className="text-[12px] text-muted text-center py-6">불러오는 중...</p>)
          : comments.length === 0 ? (<p className="text-[12px] text-muted text-center py-6">첫 댓글을 남겨보세요. (+0.5 mlbg)</p>)
          : (
