  ...
  return (
    <div className="border border-cyan/40 bg-cyan/5 px-4 py-3">
      ...
      <div className="flex items-baseline justify-between gap-2 mb-2 flex-wrap">
        ...
        {info.price != null && (
          <div className="flex items-baseline gap-2">
            <span className="text-[18px] font-bold text-text tabular-nums">{fmtPrice(info.price, info.foreign, info.currency)}</span>
            {pct != null && (
      ...
      {info.after && info.after.price != null && (
        <div className="text-[11px] text-muted mb-2 flex items-center gap-1.5 flex-wrap">
          <span className="px-1.5 py-px bg-navy/5 text-navy font-semibold rounded">{sessionLabel}</span>
          <span className="font-semibold text-text tabular-nums">{fmtPrice(info.after.price, info.foreign, info.currency)}</span>
      ...
      {(info.open != null || info.high != null || info.low != null) && (
        <div className="grid grid-cols-3 gap-x-3 text-[11px] mb-1.5 pb-1.5 border-b border-border/40">
          {info.open != null && <div className="flex justify-between"><span className="text-muted">시가</span><span className="font-semibold tabular-nums">{fmtPrice(info.open, info.foreign, info.currency)}</span></div>}
          {info.high != null && <div className="flex justify-between"><span className="text-muted">고가</span><span className="font-semibold tabular-nums text-[#dc2626]">{fmtPrice(info.high, info.foreign, info.currency)}</span></div>}
          {info.low != null && <div className="flex justify-between"><span className="text-muted">저가</span><span className="font-semibold tabular-nums text-[#2563eb]">{fmtPrice(info.low, info.foreign, info.currency)}</span></div>}
        </div>
      )}
      ...
      <div className={`grid ${compact ? 'grid-cols-2' : 'grid-cols-2 lg:grid-cols-4'} gap-x-3 gap-y-1 text-[11px]`}>
        ...
        {info.tradingValue && <div className="flex justify-between"><span className="text-muted">거래대금</span><span className="font-semibold tabular-nums">{info.tradingValue}</span></div>}
        ...
        {!compact && (
          <>
            {info.eps && <div className="flex justify-between"><span className="text-muted">EPS</span><span className="font-semibold tabular-nums">{info.eps}</span></div>}
            {info.bps && <div className="flex justify-between"><span className="text-muted">BPS</span><span className="font-semibold tabular-nums">{info.bps}</span></div>}
            {info.high52 != null && <div className="flex justify-between"><span className="text-muted">52주 최고</span><span className="font-semibold tabular-nums">{fmtPrice(info.high52, info.foreign, info.currency)}</span></div>}
            {info.low52 != null && <div className="flex justify-between"><span className="text-muted">52주 최저</span><span className="font-semibold tabular-nums">{fmtPrice(info.low52, info.foreign, info.currency)}</span></div>}
            ...
            {info.dividendYield && info.dividendYield !== '0.00%' && info.dividendYield !== '0.00' && <div className="flex justify-between"><span className="text-muted">배당수익률</span><span className="font-semibold tabular-nums">{info.dividendYield}</span></div>}
            {info.dividend && info.dividend !== '0.00' && info.dividend !== '0' && <div className="flex justify-between"><span className="text-muted">배당금</span><span className="font-semibold tabular-nums">{info.dividend}</span></div>}
          </>
        )}
      </div>
    </div>
