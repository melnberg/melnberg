...
export default function KidsDetailClient({ pin }: { pin: KidsItem }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: cms } = await supabase.from('kids_pin_comments')
  ...
  async function toggleLike() {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.rpc('toggle_kids_pin_like', { p_pin_id: pin.id });
    setBusy(false);
  ...
  async function submitComment(e: React.FormEvent) {
    ...
    if (busy) return;
    setBusy(true);
    const { data, error } = await supabase.from('kids_pin_comments')
    ...
    <article>
      <header className="pb-4 mb-6 border-b border-border">
        ...
        <div className="text-[11px] text-muted mt-1 flex items-center gap-2">
          ...
          {isAuthor && <Link href={`/kids/${pin.id}/edit`} className="text-cyan underline hover:text-navy no-underline">✏ 수정</Link>}
        </div>
      </header>
      ...
      <div className="border-l-4 border-[#ec4899] bg-[#fdf2f8] px-4 py-3 mb-6">
        <div className="text-[11px] font-bold tracking-widest uppercase text-[#ec4899] mb-1">추천 액티비티</div>
      ...
      <div className="border border-border bg-bg/30 px-4 py-3 mb-8 text-[13px]">
        {pin.occupier_id ? (
          ...
          <div>분양가 <b className="text-navy">{Number(pin.occupy_price).toLocaleString()} mlbg</b> · 일 수익 {pin.daily_income} mlbg · <a href={`/?kids=${pin.id}`} target="_top" className="text-cyan underline">지도에서 분양받기</a></div>
        )}
      </div>
