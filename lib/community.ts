...
export type PostCategory = 'community' | 'blog' | 'hotdeal' | 'stocks' | 'realty' | 'worry' | 'coin' | 'love';
...
export async function listComments(postId: number): Promise<CommunityComment[]> {
  const supabase = await createClient();
  // apt_count 까지 메인 join 에 포함 — 별도 round-trip 제거. apt_count 컬럼 없으면 (SQL 062 미적용)
  // 에러 → fallback 으로 apt_count 빼고 재시도.
  ...
  if (!primary.error && primary.data) {
    return primary.data as unknown as CommunityComment[];
  }
  const fb = await supabase
    ...
    .order('created_at', { ascending: true });
  if (fb.error) {
    console.error('listComments error', fb.error);
    return [];
  }
  return (fb.data ?? []) as unknown as CommunityComment[];
}
