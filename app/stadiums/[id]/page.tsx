...
export default async function StadiumDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase.rpc('list_recent_stadium_pins', { p_limit: 1000 });
  ...
  return (
    <Layout>
      <MainTop crumbs={[{ href: '/', label: '멜른버그' }, { href: '/stadiums', label: '경기장·운동장' }, { label: fullName, bold: true }]} meta="Stadium" />
      <section className="py-8">
        <div className="max-w-[760px] mx-auto px-6">
          <StadiumDetailClient pin={pin} />
          <div className="mt-8 text-center">
            <Link href="/stadiums" className="text-[12px] text-muted no-underline hover:text-navy">← 경기장·운동장 목록</Link>
          </div>
        </div>
      </section>
    </Layout>
