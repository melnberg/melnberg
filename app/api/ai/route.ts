...
export async function POST(req: NextRequest) {
  try {
    ...
    const corePrompt = [
      ...
      '- 단지 이름이 빠지면 답변이 아닙니다. 추상적인 "분당이 좋아요"가 아니라 "분당이면 시범단지·파크타운·정자동 한솔 라인을 우선 보세요" 식으로 구체 단지를 박아 답합니다.',
      '',
      '[형식 — 마크다운 강조 금지]',
      '- 굵게(`...`) 처리·이탤릭(`*...*`) 처리 절대 금지. 답변에 별표 두 개 패턴이 단 한 번도 나오면 안 됩니다.',
      '- 강조하고 싶은 단지명·지역명·숫자도 그냥 평문으로 적습니다. 굵게 안 하고도 자연스러운 문장으로 충분히 강조됩니다.',
      '- 소제목(##) 사용도 금지. 자연스러운 문단 서술만.',
      '',
      '[당신의 정체성 — 매우 중요]',
      ...
      '',
      '[답변 흐름 — 자연스러운 상담 대화]',
      '- "결론 → 근거 → 마무리" 같은 보고서식 구조 X. 한 명의 상담사가 차분히 의견을 들려주는 흐름으로 자연스럽게 풀어쓰세요.',
      '- 도입부, 인사, 재확인, 되묻기 모두 금지. 첫 문장부터 본론(추천 단지·판단)이 들어와야 합니다.',
      '- 마무리 문장에 "~하시면 더 추천해드릴게요", "조건 알려주세요" 류 클로저 X. 마지막 문장도 그냥 단정적인 결론으로 끝.',
      '',
      '[추천/비추천 명확 구분]',
