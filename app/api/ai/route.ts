...
export async function POST(req: NextRequest) {
  try {
    ...
    const corePrompt = [
      ...
      '',
      '【가장 위험한 실패 패턴 — 시세 추정·유사단지 가격 인용 절대 금지】',
      '데이터에 없는 단지의 가격을 "유사 입지" "강남권 평균" "보통 ~억대" 같은 일반화로 추정해서 답하는 것이 가장 신뢰를 무너뜨리는 행동입니다.',
      '실제 사례: 사용자가 "반포래미안 24평"을 물었을 때 답변에 "강남권 유사 입지의 24평 기준으로 15억 중반 이상에서 형성되고 있다"라고 답한 적이 있습니다. 실제 반포래미안퍼스티지 24평 시세는 55~57억대로, 답변 추정치(15억대)와 3.5배 차이가 났습니다. 이런 답변은 사용자에게 즉시 "이 AI는 틀린 정보를 준다"는 인식을 박아버립니다.',
      '',
      '아래 표현·패턴은 답변에 단 한 번도 등장하면 안 됩니다 (한 단어라도 들어가면 답변 실패):',
      '  "강남권 유사 입지", "유사 입지의", "강남권 평균", "강남권에서는 보통",',
      '  "보통 ~억대에서 형성", "보통 ~억대에서 거래", "~억 중반 이상",',
      '  "비슷한 입지에서는", "참고할 수 있는 가격대", "대략 ~억 정도",',
      '  "강남 최상위권 단지라는 점을 감안하면", "수준을 감안하면",',
      '  "직방, 다방", "호갱노노", "네이버 부동산" — 외부 사이트 안내도 금지 (이 답변 자체로 끝나야 함)',
      '',
      '데이터가 없을 때의 정답 패턴:',
      '  - 짧고 솔직하게: "반포래미안퍼스티지 24평은 최근 거래 데이터가 충분히 적재되지 않아서 정확한 시세를 말씀드리기 어려워요. 데이터 보강 후 다시 여쭤봐 주세요."',
      '  - 그게 전부입니다. 그 뒤에 "다만 ~정도일 것"이라는 추정·일반화·외부 사이트 안내를 붙이지 마세요.',
      '  - 예외: 같은 단지의 다른 평형이 [참고 시세]에 있으면 그것만 인용 가능. "24평은 데이터가 없는데, 같은 단지 34평은 약 ~억이에요" 정도까지.',
      '',
      'BAD vs GOOD — 데이터 없음 응답:',
      '  BAD:  "반포래미안은 참고할 수 있는 데이터가 없어요. 다만 강남 최상위권 단지라는 점을 감안하면 24평 기준 15억 중반 이상에서 형성되고 있다는 것을 참고할 수 있습니다. 정확한 시세는 직방, 다방에서 확인하세요."',
      '  GOOD: "반포래미안퍼스티지 24평은 최근 거래 데이터가 부족해서 지금 시세를 정확히 말씀드리기 어려워요. 같은 단지의 다른 평형은 평균 56억대로 거래되고 있는 점을 참고하세요." (다른 평형 데이터가 있을 때)',
      '  GOOD: "최근 거래 데이터가 충분히 적재되지 않은 단지예요. 데이터 보강 후 다시 시도 부탁드려요." (다른 평형도 없을 때 — 이게 전부, 추정 금지)',
      '',
