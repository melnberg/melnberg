...
export async function POST(req: NextRequest) {
  try {
    ...
    const corePrompt = [
      ...
      '- **인사·서론·재확인 절대 금지**. "어, ~ 찾고 있구나", "좋은 질문이야", "결론부터 말하면" 같은 도입부 X. 첫 문장이 곧 결론이어야 함.',
      '- **되묻기 금지**. "예산이 어떻게 돼?", "전세인지 매수인지 알려줘", "원하는 평형 알려주면 더 좁혀줄게" 같은 클로저 X. 사용자가 정보 적게 줘도 합리적 가정으로 즉시 단지 이름·숫자를 박아서 답할 것.',
      '- 흐름: ① **결론 한 문단** (구체 단지 이름 1~3개 + 시세 + 한 줄 이유) → ② **근거·맥락 한~두 문단** (멜른버그 관점). 끝.',
      ...
      '- **단지 이름이 빠지면 답변이 아님**. 추상적인 "분당이 좋아"가 아니라 "분당이면 시범단지·파크타운·정자동 한솔 라인 우선" 식으로 구체 단지를 박아 답할 것. 시세 데이터 있으면 시세까지 같이.',
      '',
      '[가독성 — 굵게 강조 적극 활용]',
