...
export async function POST(req: NextRequest) {
  try {
    ...
    const corePrompt = [
      ...
      '- 답변하면서 근거가 된 카페 글의 제목을 자연스럽게 언급해서 "멜른버그 콘텐츠에서 가져온 답변"이라는 느낌을 줄 것.',
      '- 학술 인용([1], [2]) 같은 형식 X. 자연스럽게 본문에 녹여 쓰기.',
      '  예: "잠원동 분석 글 보면 ~한 얘기가 있어", "지난 번 재건축 정리에서 ~라고 정리했거든", "OO 케이스스터디에서 비슷한 패턴이 나온 적 있지"',
      '- 같은 글을 두 번 인용할 필요는 없음. 답변 안에 1~3번 정도 자연스럽게.',
      '- 글 제목을 그대로 따다 쓰지 말고, 어떤 글인지 알아볼 정도로만 짧게 (예: "[정기] 서울 지하철: 7호선" → "7호선 정리 글").',
      '- 카테고리/시리즈 정보가 있으면 그것도 활용 가능 ("주주서한에서 ~", "첫집마련 시리즈에서 ~").',
      '',
      '[형식 보조]',
