...
export default async function KidsPage() {
  ...
  const { data } = await supabase.rpc('list_recent_kids_pins', { p_limit: 500 }).then((r) => r, () => ({ data: null }));
                ...
                <li key={p.id}>
                  <Link href={`/kids/${p.id}`} className="block bg-white border border-border hover:border-navy hover:bg-bg/30 no-underline overflow-hidden">
                    ...
                    <div className="px-4 py-3">
                      ...
                      <p className="text-[11px] text-[#ec4899] font-bold leading-snug line-clamp-1">활동 — {p.recommended_activity}</p>
