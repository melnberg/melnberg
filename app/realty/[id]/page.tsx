...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  const post = await getPost(Number(id), 'realty');
  if (!post) return {};
  return {
    title: `${post.title} — 부동산 토론 — 멜른버그`,
    description: post.content.slice(0, 140),
  };
}

export default async function RealtyPostDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const numId = Number(id);
  if (!Number.isFinite(numId)) notFound();

  const [post, comments] = await Promise.all([getPost(numId, 'realty'), listComments(numId)]);
  if (!post) notFound();
      ...
      <Layout current="realty">
        ...
        <article className="py-24">
          <div className="max-w-[520px] mx-auto px-6 text-center">
            ...
            <Link href="/realty" className="inline-block bg-cyan text-navy px-6 py-3 text-[13px] font-bold tracking-wide no-underline hover:bg-cyan/80">
              ← 부동산 토론 목록
            </Link>
          </div>
        </article>
          ...
          <header className="pb-6 mb-6 border-b border-cyan/30">
            <div className="flex items-start justify-between gap-3 mb-2">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="inline-block bg-cyan/10 text-navy text-[10px] font-bold tracking-widest uppercase px-2 py-0.5">🏢 REALTY</span>
              </div>
