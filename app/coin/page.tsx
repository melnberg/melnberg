...
export default async function CoinPage() {
  const [posts, user] = await Promise.all([listPosts('coin'), getCurrentUser()]);
      ...
      <section className="pt-8 lg:pt-14 pb-2">
        <div className="max-w-content mx-auto px-4 lg:px-10">
          <div className="flex items-center justify-between gap-4 pb-3 border-b-2 border-cyan">
            <div>
              <h1 className="text-[24px] lg:text-[32px] font-bold text-navy tracking-tight">₿ 코인 토론</h1>
              <p className="text-[12px] text-muted mt-1">코인·암호화폐 자유 토론. 보상은 일반 커뮤글과 동일.</p>
            </div>
            {user && (
              <Link
                href="/coin/new"
      ...
      <section className="py-6">
        <div className="max-w-content mx-auto px-4 lg:px-10">
          <div className="mb-4 px-4 py-3 bg-cyan/5 border border-cyan/30 text-[12px] leading-relaxed text-text">
            <b className="text-navy">코인 토론 부여 규칙</b>
            <ul className="mt-1.5 space-y-0.5 list-disc list-inside">
              <li>글 작성: <b>+2 mlbg</b> · 댓글: <b>+0.5 mlbg</b> · 게시글 농사 +0.5 mlbg</li>
              <li>코인 종목 자유 토론 — BTC, ETH, 알트, 디파이 등 무엇이든.</li>
            </ul>
          </div>
          {posts.length === 0 ? (
            <div className="text-center py-20">
              ...
              {user ? (
                <Link href="/coin/new" className="inline-block bg-cyan text-navy px-6 py-3 text-[13px] font-bold tracking-wide no-underline hover:bg-cyan/80">
                  첫 글 쓰기 →
                </Link>
              ) : (
                <Link href="/login?next=/coin/new" className="inline-block border-2 border-cyan text-navy px-6 py-3 text-[13px] font-bold tracking-wide no-underline hover:bg-cyan hover:text-navy">
                  로그인하고 글쓰기
                </Link>
              )}
            </div>
