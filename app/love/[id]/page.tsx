...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  const post = await getPost(Number(id), 'love');
  if (!post) return {};
  return {
    title: `${post.title} — 연애상담 — 멜른버그`,
    description: post.content.slice(0, 140),
  };
}

export default async function LovePostDetail({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const numId = Number(id);
  if (!Number.isFinite(numId)) notFound();

  const [post, comments] = await Promise.all([getPost(numId, 'love'), listComments(numId)]);
  if (!post) notFound();
      ...
      <Layout current="love">
        ...
        <article className="py-24">
          <div className="max-w-[520px] mx-auto px-6 text-center">
            ...
            <Link href="/love" className="inline-block bg-cyan text-navy px-6 py-3 text-[13px] font-bold tracking-wide no-underline hover:bg-cyan/80">
              ← 연애상담 목록
            </Link>
          </div>
        </article>
  ...
  // love 게시판은 댓글 작성자도 익명으로 표시. CommentSection 에 currentUserName="익명" 전달.
        ...
        <div className="max-w-[760px] mx-auto px-6">
          <header className="pb-6 mb-6 border-b border-cyan/30">
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <span className="inline-block bg-cyan/10 text-navy text-[10px] font-bold tracking-widest uppercase px-2 py-0.5">❤ LOVE</span>
            </div>
            ...
            <div className="flex items-center gap-3 text-[12px] text-muted flex-wrap">
              ...
              {isAuthor && (<><span>·</span><PostActions postId={post.id} basePath="/love" /></>)}
            </div>
          </header>
          ...
          <div className="mt-10 pt-6 border-t border-border flex justify-between items-center">
            <Link href="/love" className="text-[13px] font-bold text-navy no-underline hover:underline">
              ← 목록으로
            </Link>
          </div>
        </div>
