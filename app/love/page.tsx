...
export const metadata = {
  title: '연애상담 (익명) — 멜른버그',
  description: '노련한 멜른버그 유부의 현명한 답변을 얻어 보세요',
};

export const dynamic = 'force-dynamic';

export default async function LovePage() {
  const [posts, user] = await Promise.all([listPosts('love'), getCurrentUser()]);

  return (
    <Layout current="love">
      <MainTop crumbs={[{ href: '/', label: '멜른버그' }, { href: '/love', label: '연애상담 (익명)', bold: true }]} meta="Love" />
      ...
      <section className="pt-8 lg:pt-14 pb-2">
        <div className="max-w-content mx-auto px-4 lg:px-10">
          <div className="flex items-center justify-between gap-4 pb-3 border-b-2 border-cyan">
            <div>
              <h1 className="text-[24px] lg:text-[32px] font-bold text-navy tracking-tight">❤ 연애상담 (익명)</h1>
              <p className="text-[12px] text-muted mt-1">노련한 멜른버그 유부의 현명한 답변을 얻어 보세요</p>
            </div>
            <Link
              href={user ? '/love/new' : '/login?next=/love/new'}
      ...
      <section className="py-6">
        <div className="max-w-content mx-auto px-4 lg:px-10">
          <div className="mb-4 px-4 py-3 bg-cyan/5 border border-cyan/30 text-[12px] leading-relaxed text-text">
            <b className="text-navy">연애상담 규칙</b>
          ...
          {posts.length === 0 ? (
            <div className="text-center py-20">
              ...
              {user ? (
                <Link href="/love/new" className="inline-block bg-cyan text-navy px-6 py-3 text-[13px] font-bold tracking-wide no-underline hover:bg-cyan/80">
                  첫 글 쓰기 →
                </Link>
              ) : (
                <Link href="/login?next=/love/new" className="inline-block border-2 border-cyan text-navy px-6 py-3 text-[13px] font-bold tracking-wide no-underline hover:bg-cyan hover:text-navy">
                  로그인하고 글쓰기
                </Link>
              )}
            </div>
